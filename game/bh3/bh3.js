...
export class bh3 extends mysApi {
    ...
    get region() {
        for (const data of this.UserGameRoles["bh3_cn"]) {
    ...
    get uids() {
        return this.UserGameRoles['bh3_cn'].map((data) => data.game_uid)
    }
    constructor(cookie) {
        super(cookie);
    }
    ...
    async getUserGameRoles() {
        ...
        return this.UserGameRoles["bh3_cn"] || null
    }
    async sign(game_uid = '') {
        let th = this;
        ...
        if (game_uid == '') {
            let gameRoles = this.UserGameRoles[this.game_biz];
            let retdata = [];
